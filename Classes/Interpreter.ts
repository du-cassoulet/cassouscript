...
export default class Interpreter {
  ...
  public visit_VarReAssignNode(node: VarReAssignNode, context: Context) {
    ...
    const newValue = res.register(this.visit(node.newValueNode, context));
    let value = context.symbolTable?.get(varName);
          ...
          if (i < node.keys.length - 1) {
            ...
            switch (assignTok) {
              ...
              case TokenTypes.PLUSEQ:
                [error, cleanValue] = value.entries[key].addedTo(newValue);
                break;

              case TokenTypes.MINUSEQ:
                [error, cleanValue] = value.entries[key].subbedBy(newValue);
                break;

              case TokenTypes.MULEQ:
                [error, cleanValue] = value.entries[key].multedBy(newValue);
                break;

              case TokenTypes.DIVEQ:
                [error, cleanValue] = value.entries[key].divedBy(newValue);
                break;
            }

            if (error) res.failure(error);
            value.entries[key] = newValue;
            context.symbolTable?.set(varName, value);
          }
        } else {
          return res.failure(
